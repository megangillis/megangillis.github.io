...
ggplot(data = station, aes (x = current_dockcount)) +
  geom_bar(fill = "lightgreen", color = "blue") +
  theme_classic() +
...
start_date_ym %>%
  ...
  xlim(1,100) +
  facet_grid(.~ usertype) + 
  scale_fill_manual(values = c("blue violet", "chartreuse")) +
  theme_bw() +
  guides(fill = FALSE) +
  geom_vline(aes(xintercept = quantile(tripduration/60, prob = 0.75)), linetype = "dashed")
...
#
```{r, message= FALSE}
tripduration_m <- start_date_ym$tripduration
...
ggplot(aes(x = cost, fill = usertype)) +
  geom_histogram() +
  facet_grid(.~ usertype) +
  scale_fill_manual(values = "pink", "lightblue") +
  guides(fill = F) +
  theme_bw() +
...
hist(trip_age$age, main = "Member Age", xlab = "Number of Riders", col = "lightpink", breaks = 25)
