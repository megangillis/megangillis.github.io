---
title: "Seattle's Bicycle Sharing Via Graphical Analyses"
...
##Stations Locations Data (zoomed in)

```{r, label = "closeUp", warning=FALSE, message=FALSE, echo = FALSE}
...
```{r, label = "nbs", echo = FALSE, message=FALSE, warning = FALSE}
...
```{r, label = "visualRidesPerDay", message=FALSE, echo = FALSE}
...
```{r, label = "monthSeason", message=FALSE, echo = FALSE}
...
```{r, label = "minutesSeason", message=FALSE, echo = FALSE}
...
```{r, eval = FALSE, label = "dayOfWeekSeasons", message=FALSE, echo = FALSE}
start_date_ym %>%
df <- as.Date(start_date_ym$start_date) %>%
wd1 <- wday(df, label=TRUE) %>%

wd <- c("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", 
    "Friday", "Saturday")[as.POSIXlt(start_date_ym$start_date)$wday + 1] %>%


start_date_ym %>%
  group_by(wd1, Season) %>%
  summarize(N = n())%>%
  ggplot(aes(x = wd1, y = N, col = Season, fill = Season, group = Season)) + 
  geom_point() + 
  geom_line() + 
  theme_bw() + 
...
```{r}
as.POSIXlt(start_date_ym$start_date)$wday
```