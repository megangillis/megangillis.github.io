...
We can find the original information [here](https://proquestionasker.github.io/projects/2016/11/10/bicyclesseattle/).
...
#For show (the graphs, the evidence) Formating the dates
...
trip22 %>% mutate(se = sd/sqrt(n()), Season = ifelse(ym %in% c("2014/10", "2014/11"), "Fall",
                                     ifelse(ym %in% c("2014/12", "2015/1", "2015/2"), "Winter",
                                     ifelse(ym %in% c("2015/3", "2015/4", "2015/5"), "Spring", "Summer"))))
...
ggplot(data = trip22, aes (x = wd, y = tripduration_m), col = Season, group = Season) +
  ...
  theme_classic() +
  labs( x = "Weekday", y= "count", title = "Trips Per Day")
