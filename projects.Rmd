...
---
```{r, eval = FALSE}
"The website where the orignial project was completed:"
...
```
## **Introduction**
This is a report and analysis on Seattle's bicycle sharing trends. The data includes weather reports for the area (excluding summer logistics), the station locations, as well as trips taken by cycle riders. 
Future explorations of this type of data could include investigating more extensively general public transportation data trends for the Seattle area. This would serve to better understand overall, and perhaps more "modern" commuter travel trends.

## **Station Locations**
Lets take a look at where the rental stations are located in Seattle!
```{r, label = "stationLocs", "Station Locations"}
...
```{r, label = "closeUp", fig.cap="Stations map Zoomed In"}
...
```{r, label = "bikesPerStation", fig.cap="Change in Number of Bike Docks Per Station"}
...
```{r, label = "visualRidesPerDay", fig.cap="Time to visualize the number of rides per day"}
...
##Sam Look here!
```{r, label = "dayOfWeekSeasons", fig.cap="Number of Trips by Day of Week"}
